...
`spec.type` is an optional field that indicates the type of the app that this `AppBinding` is pointing to. Stash uses this field to resolve the values of `TARGET_APP_TYPE`, `TARGET_APP_GROUP` and `TARGET_APP_RESOURCE` variables of [BackupBlueprint](/docs/v2020.10.21/concepts/crds/backupblueprint) object.
...
- Learn how to backup/restore databases using Stash from [here](/docs/v2020.10.21/guides/latest/addons/overview).

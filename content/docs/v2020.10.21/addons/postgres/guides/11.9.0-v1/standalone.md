...
menu:
  docs_v2020.10.21:
    identifier: standalone-postgres-11.9.0-v1
    name: Standalone PostgreSQL
    parent: stash-postgres-guides-11.9.0-v1
    weight: 10
product_name: stash
...
sample-postgres   11.9      Running   3m11s
```
...
Here, we have to use service `sample-postgres` and secret `sample-postgres-auth` to connect with the database. KubeDB creates an [AppBinding](/docs/v2020.10.21/concepts/crds/appbinding) crd that holds the necessary information to connect with the database.
...
metadata:
  ...
  labels:
    ...
    app.kubernetes.io/version: "11.9"
...
We are going to store our backed up data into a GCS bucket. At first, we need to create a secret with GCS credentials then we need to create a `Repository` crd. If you want to use a different backend, please read the respective backend configuration doc from [here](/docs/v2020.10.21/guides/latest/backends/overview).
...
NAME                VERSION   STATUS         AGE
restored-postgres   11.9      Initializing   3m21s
```
...
$ kubectl get pg -n demo restored-postgres
NAME                VERSION   STATUS    AGE
restored-postgres   11.9      Running   2m16s
```

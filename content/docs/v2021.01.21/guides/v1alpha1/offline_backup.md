...
This tutorial will show you how to backup a Kubernetes deployment using Stash in offline mode. By default, stash takes backup in [online](/docs/v2021.01.21/guides/v1alpha1/backup) mode where sidecar container is added to take periodic backups and check backups. But sometimes you need to ensure that source data is not being modified while taking the backup, that means running backup while keeping workload pod stopped. In such case, you can run the backup in offline mode. To do this you need to specify `spec.type: offline` in `Restic` crd.
...
>Note: YAML files used in this tutorial are stored in [/docs/examples/backup](/docs/v2021.01.21/examples/backup) directory of [stashed/docs](https://github.com/stashed/docs) repository.
...
<p align="center">
  <img alt="Stash Offline Backup Flow" src="/docs/v2021.01.21/images/v1alpha1/stash-offline-backup.svg">
</p>
...
Here, we are going to backup the `/source/data` folder of a `busybox` pod into an [NFS](https://kubernetes.io/docs/concepts/storage/volumes/#nfs) volume. NFS volume is a type of [local](/docs/v2021.01.21/guides/v1alpha1/backends/local) backend for Stash.

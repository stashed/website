...
menu:
  docs_v2020.11.17:
    identifier: standalone-postgres-11.9.0-v3
    name: Standalone PostgreSQL
    parent: stash-postgres-guides-11.9.0-v3
    weight: 10
product_name: stash
...
- Install PostgreSQL addon for Stash following the steps [here](/docs/v2020.11.17/addons/postgres/setup/install)
...
- If you are not familiar with how Stash backup and restore PostgreSQL databases, please check the following guide [here](/docs/v2020.11.17/addons/postgres/overview):
...
- [RestoreSession](/docs/v2020.11.17/concepts/crds/restoresession)
...
Here, we have to use service `sample-postgres` and secret `sample-postgres-auth` to connect with the database. KubeDB creates an [AppBinding](/docs/v2020.11.17/concepts/crds/appbinding) crd that holds the necessary information to connect with the database.
...
We are going to store our backed up data into a GCS bucket. At first, we need to create a secret with GCS credentials then we need to create a `Repository` crd. If you want to use a different backend, please read the respective backend configuration doc from [here](/docs/v2020.11.17/guides/latest/backends/overview).

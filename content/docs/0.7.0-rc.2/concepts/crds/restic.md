...
### spec.type
The default value for `spec.type` is `online`. For offline backup you need to specify `spec.type=offline`. For more details see [here](/docs/0.7.0-rc.2/guides/offline_backup).
...
### spec.backend
To learn how to configure various backends for Restic, please visit [here](/docs/0.7.0-rc.2/guides/backends).
...
### spec.paused
`spec.paused` can be used as `enable/disable` switch for Restic. The default value is `false`. To stop restic from taking backup set `spec.paused: true`. For more details see [here](/docs/0.7.0-rc.2/guides/backup#disable-backup).
...
For more details about how to disable and resume Restic see [here](/docs/0.7.0-rc.2/guides/backup#disable-backup). 

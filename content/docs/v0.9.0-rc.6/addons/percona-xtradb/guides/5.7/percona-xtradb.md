...
menu:
  docs_v0.9.0-rc.6:
    identifier: percona-xtradb-guide-5.7
...
- If you are not familiar with how Stash takes backup and restores Percona XtraDB databases, please check the following guide [here](/docs/v0.9.0-rc.6/addons/percona-xtradb/overview).
...
Here, we have to use service `sample-xtradb` and secret `sample-xtradb-auth` to connect with the database. KubeDB creates an [AppBinding](/docs/v0.9.0-rc.6/concepts/crds/appbinding) CRD that holds the necessary information to connect with the database.

...
menu:
  docs_v0.9.0-rc.6:
    identifier: standalone-mongodb-4.2
    name: Standalone MongoDB
    parent: stash-mongodb-guides-4.2
    weight: 10
product_name: stash
...
- Install MongoDB addon for Stash following the steps [here](/docs/v0.9.0-rc.6/addons/mongodb/setup/install).
...
- If you are not familiar with how Stash backup and restore MongoDB databases, please check the following guide [here](/docs/v0.9.0-rc.6/addons/mongodb/overview).
...
sample-mongodb   4.2         Running   2m9s
```
...
Here, we have to use service `sample-mongodb` and secret `sample-mongodb-auth` to connect with the database. KubeDB creates an [AppBinding](/docs/v0.9.0-rc.6/concepts/crds/appbinding) crd that holds the necessary information to connect with the database.
...
We are going to store our backed up data into a GCS bucket. At first, we need to create a secret with GCS credentials then we need to create a `Repository` crd. If you want to use a different backend, please read the respective backend configuration doc from [here](/docs/v0.9.0-rc.6/guides/latest/backends/overview).
...
sample-mongodb-backup  mongodb-backup-4.2        */5 * * * *   true     26m
```
...
NAME               VERSION        STATUS         AGE
restored-mongodb   4.2         Initializing   17s
```
...
$ kubectl get mg -n demo restored-mongodb
NAME               VERSION        STATUS    AGE
restored-mongodb   4.2         Running   105m
```

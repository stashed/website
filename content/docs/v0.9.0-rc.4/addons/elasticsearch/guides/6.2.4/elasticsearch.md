...
- Install Elasticsearch addon for Stash following the steps [here](/docs/v0.9.0-rc.4/addons/elasticsearch/setup/install).
...
- If you are not familiar with how Stash backup and restore Elasticsearch databases, please check the following guide [here](/docs/v0.9.0-rc.4/addons/elasticsearch/overview).
...
Here, we have to use service `sample-elasticsearch` and secret `sample-elasticsearch-auth` to connect with the database. KubeDB creates an [AppBinding](/docs/v0.9.0-rc.4/concepts/crds/appbinding) crd that holds the necessary information to connect with the database.

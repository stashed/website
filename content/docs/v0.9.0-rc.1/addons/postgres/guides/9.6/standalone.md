...
- Install PostgreSQL addon for Stash following the steps [here](/docs/v0.9.0-rc.1/addons/postgres/setup/install)
...
- If you are not familiar with how Stash backup and restore PostgreSQL databases, please check the following guide [here](/docs/v0.9.0-rc.1/addons/postgres/overview):
...
Here, we have to use service `sample-postgres` and secret `sample-postgres-auth` to connect with the database. KubeDB creates an [AppBinding](/docs/v0.9.0-rc.1/concepts/crds/appbinding) crd that holds the necessary information to connect with the database.

...
- Install Percona XtraDB addon for Stash following the steps [here](/docs/v2020.10.30/addons/percona-xtradb/setup/install)
...
- If you are not familiar with how Stash takes backup and restores Percona XtraDB, please check the following guide [here](/docs/v2020.10.30/addons/percona-xtradb/overview).
...
Here, we have to use service `sample-xtradb-cluster` and secret `sample-xtradb-cluster-auth` to connect with the database. KubeDB creates an [AppBinding](/docs/v2020.10.30/concepts/crds/appbinding) CRD that holds the necessary information to connect with the database.

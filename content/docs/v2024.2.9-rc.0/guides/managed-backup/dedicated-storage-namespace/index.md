...
If you want to use a different backend, please read the doc [here](/docs/v2024.2.9-rc.0/guides/backends/overview/).

> For the GCS backend, if the bucket does not exist, Stash needs `Storage Object Admin` role permissions to create the bucket. For more details, please check the following [guide](/docs/v2024.2.9-rc.0/guides/backends/gcs/).
...
Notice the `spec.usagePolicy` section. Here, we are allowing all namespaces to refer to this repository. You can restrict this capability to a particular namespace or a group of namespaces. For more details, please follow the guide from [here](/docs/v2024.2.9-rc.0/concepts/crds/repository/).
...
At first, let's stop taking any further backup of the old StatefulSet. We are going to pause the `BackupConfiguration` that we created earlier. You can learn more how to pause a scheduled backup [here](/docs/v2024.2.9-rc.0/guides/use-cases/pause-backup/)

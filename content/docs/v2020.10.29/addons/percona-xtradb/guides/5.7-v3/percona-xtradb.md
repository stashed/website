...
- If you are not familiar with how Stash takes backup and restores Percona XtraDB databases, please check the following guide [here](/docs/v2020.10.29/addons/percona-xtradb/overview).
...
Here, we have to use service `sample-xtradb` and secret `sample-xtradb-auth` to connect with the database. KubeDB creates an [AppBinding](/docs/v2020.10.29/concepts/crds/appbinding) CRD that holds the necessary information to connect with the database.

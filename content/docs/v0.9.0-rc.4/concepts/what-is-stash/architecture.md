...
<figure align="center">
  <img alt="Stash Architecture" src="/docs/v0.9.0-rc.4/images/concepts/stash_architecture.svg">
...
- **BackupBatch**
  ...
  A `BackupBatch` is a Kubernetes `CustomResourceDefinition`(CRD) which lets you configure backup for multiple co-related stateful components(workload, database etc.) under a single configuration. For more details, please visit [here](/docs/v0.9.0-rc.4/concepts/crds/backupbatch).

- **AppBinding**

  An `AppBinding` holds necessary information to connect with a database. For more details about `AppBinding`, please visit [here](/docs/v0.9.0-rc.4/concepts/crds/appbinding).

- **Snapshot**

  A `Snapshot` is a representation of a backup snapshot in a Kubernetes native way. Stash uses Kuberentes Extended API Server for handling `Snapshot`s. For more details about `Snapshot`s, please visit [here](/docs/v0.9.0-rc.4/concepts/crds/snapshot).

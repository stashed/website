...
### spec.type
The default value for `spec.type` is `online`. For offline backup you need to specify `spec.type=offline`. For more details see [here](/docs/0.8.3/guides/offline_backup).
...
### spec.backend
To learn how to configure various backends for Restic, please visit [here](/docs/0.8.3/guides/backends/overview).
...
### spec.paused
`spec.paused` can be used as `enable/disable` switch for Restic. The default value is `false`. To stop restic from taking backup set `spec.paused: true`. For more details see [here](/docs/0.8.3/guides/backup#disable-backup).
...
Stash allow the users to provide a prefix for the backup repository directory. You can provide the prefix using  `local.subPath` for [local bckend](/docs/0.8.3/guides/backends#local) and `<backend-type>.prefix` for [other backends](/docs/0.8.3/guides/backends#aws-s3) in `spec.backend` field of `Restic` crd.
...
For more details about how to disable and resume Restic see [here](/docs/0.8.3/guides/backup#disable-backup).

## Next Steps

- Learn about Repository CRD [here](/docs/0.8.3/concepts/crds/repository)

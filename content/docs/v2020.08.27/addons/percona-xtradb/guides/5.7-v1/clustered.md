...
menu:
  docs_v2020.08.27:
    identifier: percona-xtradb-clustere-guide-5.7-v1
...
- If you are not familiar with how Stash takes backup and restores Percona XtraDB, please check the following guide [here](/docs/v2020.08.27/addons/percona-xtradb/overview).
...
Here, we have to use service `sample-xtradb-cluster` and secret `sample-xtradb-cluster-auth` to connect with the database. KubeDB creates an [AppBinding](/docs/v2020.08.27/concepts/crds/appbinding) CRD that holds the necessary information to connect with the database.

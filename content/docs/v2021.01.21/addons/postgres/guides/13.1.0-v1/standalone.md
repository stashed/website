...
menu:
  docs_v2021.01.21:
    identifier: standalone-postgres-13.1.0-v1
    name: Standalone PostgreSQL
    parent: stash-postgres-guides-13.1.0-v1
    weight: 10
product_name: stash
...
Here, we have to use service `sample-postgres` and secret `sample-postgres-auth` to connect with the database. KubeDB creates an [AppBinding](/docs/v2021.01.21/concepts/crds/appbinding) crd that holds the necessary information to connect with the database.
...
We are going to store our backed up data into a GCS bucket. At first, we need to create a secret with GCS credentials then we need to create a `Repository` crd. If you want to use a different backend, please read the respective backend configuration doc from [here](/docs/v2021.01.21/guides/latest/backends/overview).

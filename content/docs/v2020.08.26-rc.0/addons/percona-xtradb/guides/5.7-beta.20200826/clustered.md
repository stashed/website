...
menu:
  docs_v2020.08.26-rc.0:
    identifier: percona-xtradb-clustere-guide-5.7-beta.20200826
    name: Backup & Restore Percona XtraDB Cluster
    parent: stash-percona-xtradb-guides-5.7-beta.20200826
    weight: 10
product_name: stash
...
- Install Percona XtraDB addon for Stash following the steps [here](/docs/v2020.08.26-rc.0/addons/percona-xtradb/setup/install)
...
- If you are not familiar with how Stash takes backup and restores Percona XtraDB, please check the following guide [here](/docs/v2020.08.26-rc.0/addons/percona-xtradb/overview).
...
Here, we have to use service `sample-xtradb-cluster` and secret `sample-xtradb-cluster-auth` to connect with the database. KubeDB creates an [AppBinding](/docs/v2020.08.26-rc.0/concepts/crds/appbinding) CRD that holds the necessary information to connect with the database.
...
We are going to store our backed up data into a GCS bucket. At first, we need to create a secret with GCS credentials then we need to create a `Repository` CRD. If you want to use a different backend, please read the respective backend configuration doc from [here](/docs/v2020.08.26-rc.0/guides/latest/backends/overview).

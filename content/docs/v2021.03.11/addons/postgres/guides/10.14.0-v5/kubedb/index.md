...
menu:
  docs_v2021.03.11:
    identifier: standalone-postgres-10.14.0-v5-kubedb
...
- Install PostgreSQL addon for Stash following the steps [here](/docs/v2021.03.11/addons/postgres/setup/install)
...
Here, we have to use the service `sample-postgres` and secret `sample-postgres-auth` to connect with the database. KubeDB creates an [AppBinding](/docs/v2021.03.11/concepts/crds/appbinding) crd that holds the necessary information to connect with the database.
...
We are going to store our backed-up data into a GCS bucket. At first, we need to create a secret with GCS credentials then we need to create a `Repository` crd. If you want to use a different backend, please read the respective backend configuration doc from [here](/docs/v2021.03.11/guides/latest/backends/overview).
...
<figure align="center">
 <img alt="Backup data in GCS Bucket" src="/docs/v2021.03.11/kubedb/images/sample-postgres-backup.png">

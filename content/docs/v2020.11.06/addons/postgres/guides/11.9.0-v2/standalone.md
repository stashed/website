...
menu:
  docs_v2020.11.06:
    identifier: standalone-postgres-11.9.0-v2
    name: Standalone PostgreSQL
    parent: stash-postgres-guides-11.9.0-v2
    weight: 10
product_name: stash
...
Here, we have to use service `sample-postgres` and secret `sample-postgres-auth` to connect with the database. KubeDB creates an [AppBinding](/docs/v2020.11.06/concepts/crds/appbinding) crd that holds the necessary information to connect with the database.
...
We are going to store our backed up data into a GCS bucket. At first, we need to create a secret with GCS credentials then we need to create a `Repository` crd. If you want to use a different backend, please read the respective backend configuration doc from [here](/docs/v2020.11.06/guides/latest/backends/overview).

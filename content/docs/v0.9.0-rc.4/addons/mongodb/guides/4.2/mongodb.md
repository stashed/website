...
- Install MongoDB addon for Stash following the steps [here](/docs/v0.9.0-rc.4/addons/mongodb/setup/install).
...
- If you are not familiar with how Stash backup and restore MongoDB databases, please check the following guide [here](/docs/v0.9.0-rc.4/addons/mongodb/overview).
...
Here, we have to use service `sample-mongodb` and secret `sample-mongodb-auth` to connect with the database. KubeDB creates an [AppBinding](/docs/v0.9.0-rc.4/concepts/crds/appbinding) crd that holds the necessary information to connect with the database.
...
We are going to store our backed up data into a GCS bucket. At first, we need to create a secret with GCS credentials then we need to create a `Repository` crd. If you want to use a different backend, please read the respective backend configuration doc from [here](/docs/v0.9.0-rc.4/guides/latest/backends/overview).
